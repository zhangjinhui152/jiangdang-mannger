...
export default {
	...
	methods: {
		...
		getData() {
			axios
				.get(`${this.$store.state.baseUrl}${this.$store.state.selectVrUrl}`, {
					page1: 1,
					page2: 50,
				})
				.then((res) => {
		...
		handleDelete(index) {
			...
			this.$confirm("确定要删除吗？", "提示", {
				...
				.then(() => {
					console.log(
						`${this.$store.state.baseUrl}+${this.$store.state.delVrUrl}`
					);
					axios
						.post(
							`${this.$store.state.baseUrl}${this.$store.state.delVrUrl}?id=${this.tableData[index].id}`
						)
						.then(() => {
		...
		delAllSelection() {
			...
			for (let i = 0; i < length; i++) {
				...
				axios
					.post(`${this.$store.state.baseUrl}${this.$store.state.delVrUrl}`, {
